...
if (startIdx === -1 || endIdx === -1) {
  console.error("COULD NOT FIND BOUNDS IN FILE!");
  process.exit(1);
}

const actualEndIdx = endIdx + endKeyword.length;
console.log(`Bounds found: ${startIdx} to ${actualEndIdx}`);

// Let's define the new Header and Hero JS replacement
// Note that we use standard f.jsx and f.jsxs calls exactly as used in the bundled code.
...
try {
  new vm.Script(updatedJs);
  console.log("SYNTAX VALIDATION PASSED!");
  fs.writeFileSync(jsFile, updatedJs, "utf8");
  console.log("Successfully wrote updated index.js!");
} catch (e) {
